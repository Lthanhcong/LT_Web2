...
const GoogleStatergy = new LocalStrategy(
    ...
    async function (req, username, password, done) {
        const user = await User.findOne({
            $or: [{ 'google.email': username }, { email: username }],
        });
        if (user) {
            if (!user.google.email) {
                ...
                return done(
                    null,
                    newUser,
                    'User updated and Logged in Successfully with Google account.',
                );
            }
            return done(
                null,
                user,
                'Logged in Successfully with Google account.',
            );
        }
        ...
        return done(
            null,
            newUser,
            'User created and log in successfully with Google',
        );
    },
);
