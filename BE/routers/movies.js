...
router.get('/movie', async (req, res) => { //tất cả các phim
  ...
  // res.json(phim)
  res.render('category/phim', {movie, moment});
});
//phim moi duoc cong chieu
...
router.get('/cinema', async (req, res) => {
  ...
  res.render('category/rap', {premiere, cineplex, cinema, premiere_, premiere__})
})
...
router.get('/chair', async function (req, res) {
  ...
  await Ticket.findAll({
    ...
    if (ticket) {
        ...
        res.render('category/chair', {cinema, IDSuatChieu, userId, arr_Chair})
      })
    } else {
      Cinema.findOne({
        where: {
...
router.post('/book-chair', async (req, res) => {
  ...
  await Premiere.findOne({
     ...
     * @Tính_Tổng_Tiền
     */
    if (typeof lstGheDat != "undefined" && lstGheDat != null && lstGheDat.length != null &&
     lstGheDat.length > 0 && lstGheDat.length < 8 && typeof lstTinhTongTien != "undefined" && lstTinhTongTien != null &&
     lstTinhTongTien.length != null && lstTinhTongTien.length > 0 && lstTinhTongTien.length < 8) {
     lstTinhTongTien.map(item => {
      ...
      Booking.create({
        ...
        Ticket.findAll({
           ...
           */
          if (typeof ticket != "undefined" && ticket != null && ticket.length != null && ticket.length > 0) {
            /**
            ...
            Ticket.create({
                ...
                if (Ghe_trung.includes(ticket.dataValues.MaGhe)) {
                  ...
                  Booking.destroy({
                    where: {
                      MaDatCho: ticket.dataValues.DatChoMaDatCho
                    }
                  })
                  res.redirect('/category/failure')
                  /**
                  ...
                  if (Ghe_trung.length > 0 && Ghe_trung.indexOf(ticket.dataValues.MaGhe) === -1) {
                    //Xoa - Ve  Xoa- DatCho
                    Booking.destroy({
                      where: {
                        MaDatCho: ticket.dataValues.DatChoMaDatCho
                  ...
                  Ticket.findAll({
                    where: {
                      DatChoMaDatCho: ticket.dataValues.DatChoMaDatCho
                    },
                    include: [{
                      model: Booking,
                      where: {
              ...
              Ticket.create({
               ...
               Ticket.findAll({
                  ...
                  include: [{
                    ...
                    let ThoiDiemBatDau = item.Booking.Premiere.ThoiDiemBatDau
                    let GiaVe = item.Booking.premiere.GiaVe
                    let MaDatCho = item.Booking.MaDatCho
...
router.get('/premiere', async function (req, res) {
  res.render('category/premiere');
});

router.get('/detailmovie', async function (req, res) {
  res.render('category/detailmovie');
});

router.get('/chair', async function (req, res) {
  res.render('category/chair');
});
...
router.get('/trailer', async function (req, res, next) {
  ...
  Movie.findOne({
    ...
    res.render('category/trailer', {movies});
  }).catch(next);
});
...
router.get('/thongtinrap', async function (req, res, next) {
  ...
  Cineplex.findOne({
    ...
    res.render('category/cinema-information', {cineplexs});
  }).catch(next);
});
